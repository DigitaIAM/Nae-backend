...
impl OrderedTopology for StoreBatchDateTypeId {
  ...
  fn ops_for_store_goods_and_batch(
    ...
    let from: Vec<u8> =
      self.key_build(store, goods, batch.clone(), from_date, u8::MIN, UUID_NIL, false);
    let till: Vec<u8> =
      self.key_build(store, goods, batch.clone(), till_date, u8::MAX, UUID_MAX, true);
