...
use crate::services::{Services, Service, Mutation, Event, Error};
use crate::ws;
use crate::ws::{Connect, Disconnect, engine_io, error_general, socket_io, WsMessage};
...
impl Application {
    pub(crate) fn new(settings: Arc<Settings>, db: Arc<AnimoDB>) -> (Self, Receiver<Event>) {
        let services: Arc<RwLock<HashMap<String, Arc<dyn Service>>>> = Arc::new(RwLock::new(HashMap::new()));
    ...
    pub(crate) fn handle(&self, mutation: Mutation) -> crate::services::Result {
        match mutation {
            Mutation::Create(name, data, params) => {
                self.service(&name)
                  .create(data, params)
                  .map(|data| {
            ...
            Mutation::Remove(name, id, params) => {
                self.service(&name)
                  .remove(id, params)
                  .map(|data| {
    ...
    fn emit(&self, event: Event) {
        ...
        // workaround to close authentication service
...
impl Handler<ws::Event> for Commutator {
    ...
    fn handle(&mut self, msg: ws::Event, ctx: &mut Self::Context) -> Self::Result {
        ...
        let response = match msg.command.as_str() {
            ...
            "get" => {
                id_params(msg.data)
                  .and_then(|(id, params)| service.get(id, params))
            },
            "create" => {
                data_params(msg.data)
                  .and_then(|(data, params)|
                    self.app.handle(Mutation::Create(msg.path, data, params))
                  )
            },
            "update" => {
                id_data_params(msg.data)
                  .and_then(|(id, data, params)|
            ...
            "patch" => {
                id_data_params(msg.data)
                  .and_then(|(id, data, params)|
                    self.app.handle(Mutation::Patch(msg.path, id, data, params))
                  )
            },
            "remove" => {
                id_params(msg.data)
                  .and_then(|(id, params)|
                    self.app.handle(Mutation::Remove(msg.path, id, params))
                  )
            },
            _ => Err(Error::GeneralError(format!("service '{}' do not have command '{}'", msg.path, msg.command))),
        };
