...
pub(crate) struct SRefs {
  pub(crate) oid: ID,
  pub(crate) ctx: Vec<String>,

  // example: warehouse/goods/
...
impl SRefs {
  ...
  pub(crate) fn get(&self, id: &String) -> SRef {
    let year = &id[..4];
    let month = &id[6..8];
    ...
    SRef { id: id.clone(), oid: self.oid.clone(), ctx: self.ctx.clone(), path }
  }

  pub(crate) fn list(&self) -> std::io::Result<Vec<SRef>> {
    ...
    for year in years {
      ...
      for month in months {
        ...
        for record in records {
          ...
          result.push(SRef { id: id.to_string(), oid: self.oid, ctx: self.ctx.clone(), path });
        }
      }
    }
