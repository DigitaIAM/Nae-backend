...
impl Memory for AnimoDB {
  fn init(folder: PathBuf) -> Result<Self, DBError> {
    // search index
    let text_index_folder = folder.join("text_index");
    std::fs::create_dir_all(&text_index_folder).map_err(|e| DBError::from(e.to_string()))?;

    let text_search = Arc::new(TextSearch::open(&text_index_folder)?);
  ...
  fn modify(&self, mutations: Vec<ChangeTransformation>) -> Result<(), DBError> {
    ...
    // write to core storage
    {
      ...
      let wr: Result<(), DBError> = self.db.write(batch).map_err(|e| e.to_string().into());
      wr?;
    }
