...
use crate::animo::memory::Context;
...
use crate::warehouse::primitive_types::{Money, Qty};
...
impl WHStoreTopology {
  ...
  pub fn balance(
    ...
    store: Store,
    date: Time,
  ) -> Result<Memo<NamedValue<Store, Money>>, DBError> {
    let s = db.snapshot();
...
impl OperationsTopology for WHStoreTopology {
  ...
  fn depends_on(&self) -> Vec<ID> {
    vec![*SPECIFIC_OF, *STORE, *DATE, *GOODS, *QTY, *COST]
  }

  fn on_mutation(
    &self,
    tx: &mut Txn,
    cs: HashSet<(Zone, Context)>,
...
impl ToKVBytes for StoreMovement {
  fn to_kv_bytes(&self) -> Result<(Vec<u8>, AlignedVec), DBError> {
    let k =
      WHStoreTopology::position_of_operation(self.store, self.goods, self.date.clone(), &self.op);
...
impl ObjectInTopology<WHBalance, BalanceOperation, StoreMovement> for StoreBalance {
  ...
  fn apply(&self, op: &StoreMovement) -> Result<Self, DBError> {
    ...
    Ok(StoreBalance { store: self.store, goods: self.goods, date: self.date.clone(), balance })
  }
}
...
mod tests {
  ...
  fn test_store_operations() {
    ...
    assert_eq!(NamedValue::new(wh1, Money(35.into())), store_balance.value().clone());

    log::debug!("READING [2] 2022-05-28");
    let store_balance = WHStoreTopology::balance(&db, wh1, d22_05_28.clone()).expect("Ok");
    assert_eq!(NamedValue::new(wh1, Money(25.into())), store_balance.value().clone());
    ...
    let store_balance = WHStoreTopology::balance(&db, wh1, d22_05_31.clone()).expect("Ok");
    assert_eq!(NamedValue::new(wh1, Money(30.into())), store_balance.value().clone());

    log::debug!("DELETE B");
    db.modify(delete(incoming("B", "2022-05-30", wh1, g1, 2, Some(10))))
      .expect("Ok");

    log::debug!("READING 2022-05-31 [4]");
    let store_balance = WHStoreTopology::balance(&db, wh1, d22_05_31.clone()).expect("Ok");
    assert_eq!(NamedValue::new(wh1, Money(20.into())), store_balance.value().clone());
