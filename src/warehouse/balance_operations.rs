...
use crate::warehouse::primitive_types::{Money, MoneyOps, Qty};
...
impl AOperation<WHBalance> for BalanceOps {
  fn to_value(&self) -> WHBalance {
    WHBalance(&self.incoming.0 - &self.outgoing.0, &self.incoming.1 - &self.outgoing.1)
  }
}
...
impl From<&BalanceOperation> for BalanceOps {
  fn from(op: &BalanceOperation) -> Self {
    match op {
      BalanceOperation::In(qty, cost) => BalanceOps {
        incoming: (qty.clone(), cost.clone()),
        outgoing: (Qty::default(), Money::default()),
      },
      BalanceOperation::Out(qty, cost) => BalanceOps {
